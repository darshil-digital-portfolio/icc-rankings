...
function buildWelcomeMessage(userName: string, isNewUser: boolean): LocalMessage {
  ...
  return {
    ...
    role: "assistant",
    text:
      `${greeting} I'm **Twelfth Man**, your ICC Cricket Rankings assistant.\n\n` +
...
export function ChatView({ userName, googleSub, isNewUser }: ChatViewProps) {
  const welcomeMessage = buildWelcomeMessage(userName, isNewUser);
  ...
  // Load history for the current session on mount.
  useEffect(() => {
    if (historyLoaded) return;
    getHistory(sessionId)
      .then((h) => {
        if (h.messages.length > 0) {
          const loaded: LocalMessage[] = h.messages.map(
  ...
  const handleSend = useCallback(
    async (text?: string) => {
      ...
      try {
        const res = await sendMessage(msg, sessionId, googleSub);
    ...
    [input, isLoading, sessionId, googleSub],
  );
  ...
  const handleNewSession = () => {
    ...
    setHistoryLoaded(true); // Skip auto-load; it's a fresh session.
  };

  const handleSelectSession = (sid: string) => {
    setActiveSession(sid);
    setSessionId(sid);
    setHistoryLoaded(false); // Trigger history reload for the selected session.
    setMessages([]);
    setFollowups([]);
  };
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
          <div className="mx-auto max-w-3xl space-y-4">
            ...
            {isLoading && <ChatMessage role="assistant" text="" isLoading />}
