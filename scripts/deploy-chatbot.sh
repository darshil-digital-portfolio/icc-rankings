#!/usr/bin/env bash
# Deploy the Python chatbot to AWS Lambda via ECR.
# Run from the project root after merging to develop.
set -euo pipefail

AWS_REGION="ap-south-1"
AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
ECR_REPO="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/icc-rankings/icc-chatbot"
FUNCTION_NAME="icc-rankings-chatbot"
...
docker build --platform linux/amd64 --provenance=false \
  -f docker/Dockerfile.chatbot \
  -t icc-chatbot:latest \
  apps/chatbot/

echo "→ Pushing to ECR..."
docker tag icc-chatbot:latest "${ECR_REPO}:latest"
